...
function menu_animate(e){
  ...
  if(menu.width() > 60){
    $('#main').animate({'left': 0, 'width': '+=250px'},'slow');
    $('#tabs-bar').animate({'left': 0, 'width': '+=250px'},'slow');
    $(e).attr('title','打开菜单');
  }else{
    $('#tabs-bar').animate({'left':  '250px', 'width': '-=250px'},'slow');
    $('#main').animate({'left':  '250px', 'width': '-=250px'},'slow');
    $(e).attr('title','收起菜单');
  }
}
...
function pagelang(lang) {
  $.post(domain + 'admin/pagelang/changelang', {lang: lang}, function(data, status){
    if(data.code == 1){
...
$(function(){
  ...
  $('#lock-screen').click(function(){
    var lockHtml = '<div class="input-group mb-3">\
      <input type="text" class="form-control" placeholder="Please Enter Password" aria-label="" aria-describedby="basic-addon2" required>\
      <div class="input-group-append">\
        <button class="btn btn-info" type="button" onclick="lockscreen(this)">Lock</button>\
      </div>\
    </div>';
    $('#action-modal .modal-title').html('Lock-screen');
...
function lockscreen(e){
  ...
  if(psd == ''){
    Alert('Please Enter Password！', alertBg);
    return false;
  }
