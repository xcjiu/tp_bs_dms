...
class SysUser
{
  ...
  public static function edit($user, $data)
  {
    ...
    $role_id = $data['role_id'];
    AuthAssignment::where('user_id', $user->id)->update(['role_id'=>$role_id]);
    unset($data['role_id']);
  ...
  public static function getOpLogRows($params)
  {
    ...
    if($condition){
      $rows = SysOpLog::with('user')->where($condition)->order('create_time desc')->limit($offset, $limit)->select();
      $total = SysOpLog::with('user')->where($condition)->count();
    }else{
      $rows = SysOpLog::with('user')->order('create_time desc')->limit($offset, $limit)->select();
      $total = SysOpLog::count();
    }
    if($rows){
      foreach ($rows as &$value) {
        $value['username'] = $value['user']['username'];
        $value['remark'] = strlen($value['remark']) > 80 ? substr($value['remark'], 0, 80) . ' ...' : $value['remark'];
      }
    }
    return ['total'=>$total, 'rows'=>$rows];
  }
