...
class Auth
{
  ...
  public static function authAssignment(array $params)
  {
    ...
    if($title) {
      $auths = AuthRule::where('title', 'like', '%'.$title.'%')->order('type,link asc')->limit($offset, $limit)->select()->toArray();
      $total = AuthRule::where('title', 'like', '%'.$title.'%')->count();
    } else {
  ...
  public static function userAuth($uid)
  {
    ...
    if($role){
      if($role->name === '超级管理员'){
        $auth = AuthRule::field('id,type,module,pid,link,title,en_title,icon,sort')->order('sort asc')->select();
      }else{
        $auth = AuthRule::where('status', 1)->where('id', 'in', $role->auth_ids)->field('id,type,module,pid,link,title,en_title,icon,sort')->order('sort asc')->select();
      }
    }else{
      $auth = AuthRule::where('link','admin/index/index')->field('id,type,module,pid,link,title,en_title,icon,sort')->select(); //如果没有分配权限，则只显示首页
    }
    return $auth;
  }
